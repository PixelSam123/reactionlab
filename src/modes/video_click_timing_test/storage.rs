...
pub fn data_dir() -> PathBuf {
    dirs::data_dir()
        ...
        .join("reactionlab")
        .join("video_click_timing_test")
}

pub fn config_path() -> PathBuf {
    data_dir().join("video_groups.json")
}

pub fn load_config() -> GroupConfig {
...
pub fn save_config(config: &GroupConfig) {
    let path = config_path();
        ...
        for entry in read_dir.flatten() {
                ...
                && let Ok(run_data) = serde_json::from_str::<RunData>(&content)
            {
                let errors: Vec<f64> = run_data
                    .attempts
                    .iter()
                    .filter(|round| round.outcome.is_hit())
